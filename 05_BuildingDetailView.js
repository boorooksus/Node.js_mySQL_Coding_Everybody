...
// node.js 어플리케이션이 mysql에서 가져온 데이터를 통해 상세보기 페이지 구현
// 1: 변경 내용
// 2: "db.query(`SELECT * FROM topic WHERE id=${queryData.id}`, function( ~ "
//  이렇게 써도 되지만 공격을 받을 수 있기 때문에 queryData.id 대신 물음표로 쓰고 queryData.id는 두 번째 인자로 전달해야 안전함.
//  물음표는 두 번째 인자로 치환하기로 약속되어 있음.